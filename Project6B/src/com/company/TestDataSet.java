...
public class TestDataSet {
    ...
    public TestDataSet(NaiveBayes nb, Instances dataAll, Instances distribution) throws Exception{
        ...
        NumericToNominal ntm = new NumericToNominal();      //Converto in nominali gli attributi numerici
        ntm.setInputFormat(dataAll);
        data = ntm.useFilter(dataAll,ntm);

        nb.buildClassifier(data);               //Costruisco il classificatore Bayesiano
        ...
        for (int i = 0; i < data.numInstances(); i++) {
            ...
            ns.get(i).score = distribution.get(i).value(distribution.attribute("probability of being essential").index());     //Lo score che considero è la probabilità di appartenere alla classe Essential=Y
        }
    ...
    double getppv(int threshold) throws Exception {
        if(threshold == data.numInstances()){
            ...
            return eva.precision(data.classIndex());      //PPV per la predizione di essenzialità Y (index 0 = y)
        }
        else {
            int i = threshold +1;
            while(i<ns.size()) {
            ...
            eva = new Evaluation(data);
            eva.crossValidateModel(nb, data, 10, new Random(1));
            return eva.precision(data.classIndex());
        }
    }
